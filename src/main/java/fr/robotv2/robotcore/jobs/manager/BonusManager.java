...
public class BonusManager {
    ...
    public double applyAllBonus(Player player, Job job, Double value, Currency currency) {
        value = applyBonus(value, job, globalBonus, currency);
        value = applyBonus(value, job, personalBonus.get(player.getUniqueId()), currency);
        return value;
    }

    public void createPersonalBonus(Player player, Job job, double pourcentage, int minute, Currency currency) {
    ...
    public void createGlobalBonus(Job job, double pourcentage, int minute, Currency currency) {
        GlobalBonus bonus = new GlobalBonus(job, pourcentage, minute, currency);
        globalBonus.add(bonus);
    }

    private Double applyBonus(Double value, Job job, List<Bonus> bonusList, Currency currency) {
